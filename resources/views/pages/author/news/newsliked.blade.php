        ...
        <div class="row">
            ...
            <div class="news-card-five">
                <div class="news-card-img">
                    <img src="{{ asset('storage/' . $item->news->photo) }}" class="" width="100%" height="150px" style="object-fit: cover" alt="Image">
                </div>
                <div class="news-card-info">
                    <h3>
                        <a data-toggle="tooltip" data-placement="top" title="{{ $item->news->name }}"
                            href="{{ route('news.user', ['news' => $item->news->slug, 'year' => $dateParts['year'], 'month' => $dateParts['month'], 'day' => $dateParts['day']]) }}">{!! Illuminate\Support\Str::limit($item->news->name, $limit = 50, $end = '...') !!}</a>
                    </h3>
                    <p>
                        {!! Illuminate\Support\Str::limit(strip_tags($item->news->content), 120, '...') !!}
                    </p>
                    <ul class="news-metainfo list-style">
                    <li>
                        <i class="fi fi-rr-calendar-minus"></i><a href="javascript:view(0)">{{ \Carbon\Carbon::parse($item->news->upload_date)->format('M d Y') }}</a>
                    </li>
                    <li>
                        <i>
                        ...
                        {{-- </i><a href="javascript:void(0sing)">{{ $item->news->newsHasLikes->count() }}</a></li> --}}
                    </li>
                    </ul>
                </div>
            </div>
